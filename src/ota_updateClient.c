...
static void startUpdateDownloadWithUuid(char *targetUuidIn, size_t fwSize_bytesIn);
static void download_iterate();
...
static char targetFwUuid[UUID_LEN_BYTES+1];
static size_t currBlockIndex;
static size_t numBlocks;
static size_t blockSize_bytes;
static int numFailedTries = 0;
...
{
	// if we're currently downloading...keep doing it
	if( isDownloading )
	{
		download_iterate();
		return;
	...
	// if we made it here, we're not downloading...check for update periodically
	...
	// make sure our http client is initialized before we check
	if( !isHttpClientInit )
	{
	...
	if( updateRetVal == CHECK_FOR_UPDATE_RETVAL_UPDATE_AVAILABLE )
	{
		...
		startUpdateDownloadWithUuid(targetUuid, firmwareSize_bytes);
	}
...
static void startUpdateDownloadWithUuid(char *targetUuidIn, size_t fwSize_bytesIn)
{
	...
	numBlocks = DIV_ROUND_UP(fwSize_bytesIn, blockSize_bytes);
	OTA_LOG_INFO(TAG, "starting download of %d bytes (%d blocks)", fwSize_bytesIn, numBlocks);

	// save our FW uuid
	strncpy(targetFwUuid, targetUuidIn, sizeof(targetFwUuid));
	targetFwUuid[sizeof(targetFwUuid)-1] = 0;
}


static void download_iterate(void)
{
	char body[92];
	...
	snprintf(body, sizeof(body), "{\"targetFwUuid\":\"%s\",\"offset\":%d,\"maxNumBytes\":%d}", targetFwUuid, currBlockIndex * blockSize_bytes, blockSize_bytes);
	body[sizeof(body)-1] = 0;
